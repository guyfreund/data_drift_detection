...
class StatisticalBasedDetector(IDataDriftDetector):
    ...
    def detect(self) -> StatisticalBasedDataDrift:
        ...
        is_variance_drifted, is_mean_drifted, is_num_nulls_drifted = False, False, False

        return StatisticalBasedDataDrift(is_drifted=True)
    ...
    def _extract_data_drifts(feature_names, training_feature_metrics_list, deployment_feature_metrics_list) -> Dict[str, Dict[DataDriftType, DataDrift]]:
        ...
        for feature_name, training_fm, deployment_fm in zip(feature_names, training_feature_metrics_list, deployment_feature_metrics_list):
            ...
            is_variance_drifted = np.abs(training_variance - deployment_variance) > Config().data_drift.variance_threshold
            ...
            is_mean_drifted = np.abs(training_mean - deployment_mean) > Config().data_drift.mean_threshold
            ...
            is_num_nulls_drifted = np.abs(training_num_nulls - deployment_num_nulls) > Config().data_drift.number_of_nulls_threshold
